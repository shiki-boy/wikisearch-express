...
const Login = () => {
  ...
  const onSuccess = async ( googleResponseData ) => {
    ...
    const { data } = await httpClient.post( '/api/auth/google/verify/', { access_token: googleResponseData.credential } )

    // set jwt cookies
    const accessTokenExpires = new Date( jwtDecode( data.token ).exp * 1000 )

    Cookies.set( 'Authorization', `Bearer ${ data.token }`, { expires: accessTokenExpires } )
